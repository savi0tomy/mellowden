...
from resend_mailer import send_owner_alert
...
def build_admin_router(settings, SessionLocal, ApprovalOrder, new_token, customer_review_url, send_customer_review_email):
    ...
    router.include_router(
        build_resend_webhook_router(
            SessionLocal,
            settings=settings,
            ApprovalOrder=ApprovalOrder,
            send_owner_alert_fn=send_owner_alert,
        )
    )
...
.k{{letter-spacing:.22em;font-weight:700;font-size:12px;color:var(--muted);text-transform:uppercase}} h1{{font:700 38px/1.05 Georgia,serif;margin:12px 0 10px}} p{{color:var(--muted);line-height:1.5}} input{{width:100%;padding:14px 15px;border:1px solid var(--line);border-radius:12px;font:inherit;background:white;margin:8px 0 14px}} button{{width:100%;padding:14px;border:0;border-radius:999px;background:var(--accent);color:white;font-weight:700;font-size:15px;cursor:pointer}} .error{{background:#fff0ee;color:#8a2f25;border:1px solid #efc7c1;padding:10px 12px;border-radius:10px;margin:12px 0}}
    ...
    def timeline_html(events):
        if not events:
            return '<div class="muted tiny">No activity recorded yet.</div>'
        items = []
        for event in events[:12]:
            when = event.created_at.strftime("%Y-%m-%d %H:%M") if event.created_at else ""
            detail = f" — {escape(event.detail)}" if event.detail else ""
            items.append(f'<li><b>{escape(event.event_type.replace("_", " ").title())}</b>{detail}<span>{escape(when)} UTC</span></li>')
        return '<ul class="timeline">' + ''.join(items) + '</ul>'

    def history_html(versions):
        if not versions:
            return ""
        cards = []
        for version in versions[:8]:
            created = version.created_at.strftime("%Y-%m-%d %H:%M") if version.created_at else ""
            note = f'<div class="tiny muted">{escape(version.note)}</div>' if version.note else ""
            cards.append(
                f'<div class="version"><img src="{escape(version.artwork_url, quote=True)}" alt="Artwork version {version.version_number}"><b>v{version.version_number}</b><span>{escape(created)} UTC</span>{note}</div>'
            )
        return '<details class="history"><summary>Artwork history (' + str(len(versions)) + ')</summary><div class="versions">' + ''.join(cards) + '</div></details>'

    def dashboard_html(rows, active_status: str = "", q: str = "", message: str = "", error_message: str = "") -> str:
        cards = []
        counts = {status: 0 for status in STATUS_ORDER}
        with SessionLocal() as db:
            all_rows = db.scalars(select(ApprovalOrder)).all()
            for r in all_rows:
        ...
        for row in rows:
            ...
            with SessionLocal() as detail_db:
                delivery = latest_customer_delivery(detail_db, row.shopify_order_id)
                snapshot = get_shopify_snapshot(detail_db, row.shopify_order_id)
                versions = list_artwork_versions(detail_db, row.shopify_order_id)
                events = list_events(detail_db, row.shopify_order_id)
            ...
            shopify_url = shopify_admin_order_url(settings, row.shopify_order_id)
            live = ""
            if snapshot:
                live = f'<div class="shopifystatus"><span>Shopify payment: <b>{escape(snapshot.financial_status.replace("_", " ").title() or "—")}</b></span><span>Fulfillment: <b>{escape(snapshot.fulfillment_status.replace("_", " ").title() or "—")}</b></span></div>'

            action_buttons = []
            if row.status == "WAITING_FOR_CUSTOMER_APPROVAL":
                action_buttons.append(f'<form method="post" action="/admin/orders/{escape(row.shopify_order_id, quote=True)}/resend-review"><button type="submit" class="secondarybtn">Resend review email</button></form>')
            if row.status == "APPROVED":
                action_buttons.append(f'<form method="post" action="/admin/orders/{escape(row.shopify_order_id, quote=True)}/mark-sent-to-print-ui"><button type="submit">Mark sent to print</button></form>')

            cards.append(f"""
<article class="order">
  <div class="topline"><div><div class="ordername">{escape(row.order_name or row.shopify_order_id)}</div><div class="muted">{escape(row.product_title or '')} · {escape(row.variant_title or '')}</div>{delivery_html}{live}</div><span class="status s-{escape(row.status.lower())}">{escape(status_label(row.status))}</span></div>
  <div class="grid">
    <div>{pet_img}</div>
    <div class="details">
  ...
  <div class="linkrow"><input id="url-{escape(row.shopify_order_id, quote=True)}" value="{safe_review}" readonly><button type="button" onclick="copyUrl('url-{escape(row.shopify_order_id, quote=True)}', this)">Copy URL</button><a class="mail" href="{escape(mailto, quote=True)}">Manual email</a><a class="open" href="{safe_review}" target="_blank" rel="noopener">Open review</a><a class="open" href="{escape(shopify_url, quote=True)}" target="_blank" rel="noopener">Open in Shopify</a></div>
  <div class="actionrow">{''.join(action_buttons)}</div>
  {history_html(versions)}
  <details class="activity"><summary>Activity</summary>{timeline_html(events)}</details>
</article>""")

        empty = '<div class="emptylist">No matching portrait orders.</div>' if not cards else ""
        ...
        tabs = [f'<a class="tab {"active" if not active_status else ""}" href="/admin">All <b>{sum(counts.values())}</b></a>']
        for status in STATUS_ORDER:
            tabs.append(f'<a class="tab {"active" if active_status == status else ""}" href="/admin?status={quote(status)}">{escape(status_label(status))} <b>{counts.get(status, 0)}</b></a>')
...
main{{max-width:1280px;margin:auto;padding:34px 24px 70px}} .hero{{display:flex;align-items:end;justify-content:space-between;gap:20px;margin-bottom:20px}} h1{{font:700 clamp(34px,5vw,58px)/1 Georgia,serif;margin:0}} .sub{{color:var(--muted);margin-top:8px}} .flash{{background:var(--green);border:1px solid #c8dfcc;padding:12px 14px;border-radius:12px;margin-bottom:18px}} .errorflash{{background:#fff0ee;border-color:#efc7c1;color:#8a2f25}}
.filters{{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:18px}} .filters form{{display:flex;gap:8px;flex:1;min-width:280px}} .filters input{{flex:1;padding:11px 13px;border:1px solid var(--line);border-radius:999px;background:white}} .tabs{{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px}} .tab{{padding:9px 12px;border:1px solid var(--line);border-radius:999px;background:var(--paper);text-decoration:none;font-size:13px}} .tab.active{{background:var(--accent);color:white;border-color:var(--accent)}}
.order{{background:var(--paper);border:1px solid var(--line);border-radius:22px;padding:22px;margin:0 0 18px;box-shadow:0 14px 40px rgba(55,43,31,.04)}} .topline{{display:flex;justify-content:space-between;gap:18px;align-items:flex-start}} .ordername{{font:700 26px Georgia,serif}} .muted{{color:var(--muted);font-size:14px;margin-top:4px}} .tiny{{font-size:12px}} .status{{white-space:nowrap;padding:8px 11px;border-radius:999px;font-size:12px;font-weight:700;background:#eee5dd}} .s-approved,.s-sent_to_print{{background:var(--green)}} .s-revision_requested{{background:var(--rose)}} .s-waiting_for_customer_approval{{background:var(--amber)}} .delivery{{display:inline-block;margin-top:8px;padding:6px 9px;border-radius:999px;background:#eee5dd;font-size:11px;font-weight:700}} .d-delivered{{background:var(--green)}} .d-bounced,.d-failed,.d-complained,.d-suppressed{{background:var(--rose)}} .d-delivery_delayed{{background:var(--amber)}}
.shopifystatus{{display:flex;gap:12px;flex-wrap:wrap;margin-top:9px;font-size:12px;color:var(--muted)}} .grid{{display:grid;grid-template-columns:170px minmax(240px,1fr) 220px;gap:20px;margin-top:20px;align-items:start}} .pet,.art img{{width:100%;max-height:220px;object-fit:contain;border-radius:14px;background:#f0e9e1;border:1px solid var(--line)}} .pet.empty{{height:170px;display:grid;place-items:center;color:var(--muted)}} .details{{display:grid;grid-template-columns:1fr 1fr;gap:16px;font-size:14px;line-height:1.45}} .details b{{font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted)}} .revision{{margin-top:16px;background:var(--rose);border:1px solid #edc8c0;border-radius:12px;padding:13px 14px;line-height:1.5}}
.upload{{margin-top:18px;padding-top:18px;border-top:1px solid var(--line);display:flex;align-items:center;gap:10px;flex-wrap:wrap}} input[type=file]{{max-width:360px}} button,.mail,.open{{border:0;border-radius:999px;padding:11px 15px;font:700 13px Arial,sans-serif;cursor:pointer;text-decoration:none;display:inline-block}} button{{background:var(--accent);color:white}} .secondarybtn,.mail,.open{{background:#eee5dd;color:var(--ink)}} .linkrow,.actionrow{{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:14px}} .linkrow input{{flex:1;min-width:280px;padding:11px 12px;border:1px solid var(--line);border-radius:10px;background:white;color:var(--muted)}} .emptylist{{background:var(--paper);border:1px solid var(--line);border-radius:18px;padding:28px;color:var(--muted)}}
.history,.activity{{margin-top:16px;border-top:1px solid var(--line);padding-top:14px}} summary{{cursor:pointer;font-weight:700;font-size:13px}} .versions{{display:flex;gap:10px;overflow-x:auto;padding-top:12px}} .version{{min-width:140px;max-width:160px;font-size:12px}} .version img{{width:140px;height:110px;object-fit:contain;border:1px solid var(--line);border-radius:10px;background:#f0e9e1;display:block;margin-bottom:5px}} .version span{{display:block;color:var(--muted);font-size:11px}} .timeline{{list-style:none;padding:8px 0 0;margin:0}} .timeline li{{padding:8px 0;border-bottom:1px solid var(--line);font-size:13px}} .timeline span{{display:block;color:var(--muted);font-size:11px;margin-top:2px}}
...
<script>function copyUrl(id,btn){{const el=document.getElementById(id);navigator.clipboard.writeText(el.value).then(()=>{{const old=btn.textContent;btn.textContent='Copied';setTimeout(()=>btn.textContent=old,1200)}})}}</script>
</head><body><header><div class="head"><div class="brand">MELLOWDEN</div><a class="logout" href="/admin/logout">Sign out</a></div></header><main><div class="hero"><div><h1>Portrait orders</h1><div class="sub">Artwork, approvals, revisions, Shopify status and email delivery in one place.</div></div></div>{flash}{error_flash}<div class="filters"><form method="get" action="/admin"><input type="hidden" name="status" value="{escape(active_status, quote=True)}"><input name="q" value="{escape(q, quote=True)}" placeholder="Search order, email, pet or product"><button type="submit">Search</button></form></div><div class="tabs">{''.join(tabs)}</div>{empty}{''.join(cards)}</main></body></html>"""

    @router.get("/admin", response_class=HTMLResponse)
    def admin_home(request: Request, status: str = "", q: str = "", saved: str = "", email: str = "", action: str = ""):
        if not is_authenticated(request):
            return HTMLResponse(login_page())
        with SessionLocal() as db:
            rows = list(db.scalars(select(ApprovalOrder).order_by(ApprovalOrder.created_at.desc())).all())
        if status:
        ...
        if saved == "1" and email == "sent":
            ...
            message = "Review email sent again."
        elif action == "resent_failed":
            error_message = "Resend could not send the review email. The existing review URL is still valid."
        elif action == "print":
            message = "Order marked as sent to print."
        elif saved == "1":
            message = "Artwork saved. Review link is ready."
        return HTMLResponse(dashboard_html(rows, status, q, message, error_message))
    ...
    async def upload_artwork(request: Request, order_id: str, artwork: UploadFile = File(...)):
        ...
        with SessionLocal() as db:
            ...
            save_artwork_version(db, row, data_uri, previous_revision)
            ...
            db.refresh(row)
            review_url = customer_review_url(row.approval_token)
            record_event(db, row.shopify_order_id, "ARTWORK_UPLOADED", f"Artwork v{len(list_artwork_versions(db, row.shopify_order_id))} uploaded")
            try:
                sync_shopify_order_metadata(settings, row, review_url)
            ...
            try:
                ...
                record_event(db, row.shopify_order_id, "REVIEW_EMAIL_FAILED", str(exc)[:500])

        return RedirectResponse(f"/admin?saved=1&email={'sent' if email_sent else 'failed'}", status_code=303)

    @router.post("/admin/orders/{order_id}/resend-review")
    async def resend_review(request: Request, order_id: str):
        if not is_authenticated(request):
            return RedirectResponse("/admin", status_code=303)
        ok = False
        with SessionLocal() as db:
            ...
            if not row:
                return HTMLResponse("Order not found.", status_code=404)
            if row.status != "WAITING_FOR_CUSTOMER_APPROVAL":
                return HTMLResponse("This order is not waiting for customer approval.", status_code=409)
            try:
                send_customer_review_email(settings, row, customer_review_url(row.approval_token))
                record_event(db, row.shopify_order_id, "REVIEW_EMAIL_RESENT")
                ok = True
            except Exception as exc:
                record_event(db, row.shopify_order_id, "REVIEW_EMAIL_FAILED", str(exc)[:500])
        return RedirectResponse(f"/admin?action={'resent' if ok else 'resent_failed'}", status_code=303)

    @router.post("/admin/orders/{order_id}/mark-sent-to-print-ui")
    async def mark_sent_to_print_ui(request: Request, order_id: str):
        if not is_authenticated(request):
            return RedirectResponse("/admin", status_code=303)
        with SessionLocal() as db:
            row = db.scalar(select(ApprovalOrder).where(ApprovalOrder.shopify_order_id == order_id))
            if not row:
                return HTMLResponse("Order not found.", status_code=404)
            if row.status != "APPROVED":
                return HTMLResponse("Order must be approved before it can be sent to print.", status_code=409)
            ...
            try:
                sync_shopify_order_metadata(settings, row, customer_review_url(row.approval_token))
            except Exception as exc:
                record_event(db, row.shopify_order_id, "SHOPIFY_SYNC_FAILED", str(exc)[:500])
        return RedirectResponse("/admin?action=print", status_code=303)
