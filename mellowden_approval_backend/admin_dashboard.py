...
def build_admin_router(settings, SessionLocal, ApprovalOrder, new_token, customer_review_url):
    router = APIRouter()
...
.k{{letter-spacing:.22em;font-weight:700;font-size:12px;color:var(--muted);text-transform:uppercase}} h1{{font:700 38px/1.05 Georgia,serif;margin:12px 0 10px}} p{{color:var(--muted);line-height:1.5}} input{{width:100%;padding:14px 15px;border:1px solid var(--line);border-radius:12px;font:inherit;background:white;margin:8px 0 14px}} button{{width:100%;padding:14px;border:0;border-radius:999px;background:var(--accent);color:white;font-weight:700;font-size:15px;cursor:pointer}} .error{{background:#fff0ee;color:#8a2f25;border:1px solid #efc7c1;padding:10px 12px;border-radius:10px;margin:12px 0}}
    ...
    def dashboard_html(rows, message: str = "") -> str:
        cards = []
        for row in rows:
            ...
            cards.append(f"""
<article class="order">
  <div class="topline"><div><div class="ordername">{escape(row.order_name or row.shopify_order_id)}</div><div class="muted">{escape(row.product_title or '')} · {escape(row.variant_title or '')}</div></div><span class="status s-{escape(row.status.lower())}">{escape(status_label(row.status))}</span></div>
  <div class="grid">
    <div>{pet_img}</div>
    <div class="details">
  ...
  <form class="upload" method="post" enctype="multipart/form-data" action="/admin/orders/{escape(row.shopify_order_id, quote=True)}/upload-artwork">
    ...
    <button type="submit">Save artwork & create review link</button>
  </form>
  <div class="linkrow"><input id="url-{escape(row.shopify_order_id, quote=True)}" value="{safe_review}" readonly><button type="button" onclick="copyUrl('url-{escape(row.shopify_order_id, quote=True)}', this)">Copy URL</button><a class="mail" href="{escape(mailto, quote=True)}">Email customer</a><a class="open" href="{safe_review}" target="_blank" rel="noopener">Open review</a></div>
</article>""")
...
</head><body><header><div class="head"><div class="brand">MELLOWDEN</div><a class="logout" href="/admin/logout">Sign out</a></div></header><main><div class="hero"><div><h1>Portrait orders</h1><div class="sub">Upload artwork, send review links, and track approvals or revision requests.</div></div></div>{flash}{empty}{''.join(cards)}</main></body></html>"""

    @router.get("/admin", response_class=HTMLResponse)
    def admin_home(request: Request, saved: str = ""):
        if not is_authenticated(request):
            return HTMLResponse(login_page())
        with SessionLocal() as db:
            rows = db.scalars(select(ApprovalOrder).order_by(ApprovalOrder.created_at.desc())).all()
        message = "Artwork saved. Review link is ready to send." if saved == "1" else ""
        return HTMLResponse(dashboard_html(rows, message))
